...
import { closeSync, existsSync, openSync, readFileSync, readSync, readdirSync, realpathSync } from "fs";
...
function resolvePackageRoot(packageName = DEFAULT_PACKAGE_NAME) {
    ...
    if (envRoot && existsSync(envRoot)) return realpathSync(envRoot);
    const resolvedPath = join(process.cwd(), 'node_modules', packageName);
    if (!existsSync(resolvedPath)) throw new Error(`Package "${packageName}" not found at ${resolvedPath}. Ensure it is installed.`);
...
function readSourceFile(packageRoot, relativePath) {
    ...
    if (!existsSync(absPath)) throw new Error(`File not found: ${relativePath}`);
...
export { changelogQueryTool, componentDetailsTool, componentExamplesTool, componentListTool, componentSearchTool, createMCPServer, extractCodeBlocks, extractDescription, extractFunction, extractPropNames, extractSection, filterProps, findAllFunctions, getCodeBlockTool, getComponentFileListTool, getComponentList, getFileCodeTool, getFunctionCodeTool, getFunctionNames, getPackageVersion, handleChangelogQuery, handleComponentDetails, handleComponentExamples, handleComponentList, handleComponentSearch, handleGetCodeBlock, handleGetComponentFileList, handleGetFileCode, handleGetFunctionCode, handleThemeTokens, isLargeDocument, listComponentFiles, listTopLevelDirectories, parseExamples, parseFrontmatter, readChangelog, readComponentDoc, readDocIndex, readGuidelineDoc, readSourceFile, readThemes, readTokens, removeFunctionBodies, replaceCodeBlocksWithPlaceholders, resolvePackageRoot, searchComponents, themeTokensTool, toolHandlers, tools };
