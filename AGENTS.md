...
这是一个零外部依赖的 MCP Server 项目。所有数据（组件文档、tokens、guidelines）以本地 Markdown/JSON 文件形式打包在 `doc/` 目录下，无需数据库或外部 API。

### 如何运行

- **构建**: `npm run build`（输出 `dist/index.js`、`dist/stdio.js`、`dist/http.js`）
- **开发模式**: `npm run dev`（watch 模式，自动重新编译）
- **HTTP 服务器**: `npm run start:http`（监听 `0.0.0.0:3000`，支持 `--stateless` 无状态模式）
- **Stdio 服务器**: `npm run start`
- **测试**: `npx rstest run`（约 164 个用例，~2s 完成）
- **Lint**: 项目无独立 lint 脚本；TypeScript 类型检查通过 `rslib build` 隐含执行

### 注意事项

- HTTP 传输需要 `Accept: application/json, text/event-stream` 请求头，否则返回 406 错误
- MCP 工具参数名用 `componentName`（非 `component`），`component_search` 用 `query` 参数
- 测试日志中 `remove-function-body.test.ts` 会输出 babel parser 语法错误，属于预期行为（测试用例覆盖错误处理路径），不影响测试结果
- 源码检查工具 (`source_inspect`) 依赖 `@douyinfe/semi-ui` 包安装在 `node_modules`（通过 `npm install` 自动获取），可通过 `COMPONENT_PACKAGE_ROOT` 环境变量覆盖路径
